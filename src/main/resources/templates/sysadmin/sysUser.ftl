<@l.manage tab="sysUser" title="用户管理" import="base,vue,iview,layout" js="" >
    <div id="appUser" class="reletiveContainer list-panel-container" v-cloak>
        <div class="absoluteArea panel-head" style="">
            <div class="panel-title left">
                <i class="ivu-icon ivu-icon-ios-contact"></i>用户管理</a>
            </div>
        </div>
            ...
            <div class="reletiveContainer">
                <div class="absoluteArea func-area">
                    ...
                    <div class="right func-item" style="padding-top: 9px">
                        <i-input placeholder="点击按钮选择所属单位" :label-in-value="true" disabled
                                 v-model="searchParam.organName"><span slot="append" @click="orgSelect">单位</span>
                        </i-input>
                    </div>
                    <div class="right func-item" style="padding-top: 9px">
                        <i-input placeholder="输入用户名或登录名" v-model="searchParam.userName"><span slot="append"
        ...
        <Drawer :title="drawerTitle" width="420" :closable="false" v-model="userDrawer">
            <div class="absoluteArea reset-drawer-body" style="">
                <i-form class="form-reset" ref="userForm" :rules="userValidate" :model="user">
                    <form-item label="登录名称" prop='username'>
                        <i-input v-model="user.username" :disabled="usernameDisabled" placeholder=""></i-input>
                    </form-item>
                    <form-item label="显示名称" prop='displayName'>
                        <i-input v-model="user.displayName" :disabled="userFormDisabled" placeholder=""></i-input>
                    </form-item>
                    <#--<form-item label="用户角色" prop='userRoles' >-->
                    <#--<i-input v-model="user.displayName" :disabled="userFormDisabled" placeholder=""></i-input>-->
                    <#--</form-item>-->
                    <form-item v-if="operateType == 'add'" label="用户密码" prop="password">
                        <i-input v-model="user.password" :disabled="userFormDisabled" type="password"></i-input>
                    </form-item>
                    <form-item label="所属单位" prop="organName">
                        <i-input v-model="user.organName" disabled><span slot="append" @click="orgAdd">选择</span>
                        </i-input>
                    </form-item>
                    <form-item label="用户邮箱" prop="email">
                        <i-input v-model="user.email" :disabled="userFormDisabled" placeholder=""></i-input>
                    </form-item>
                    <form-item label="手机号" prop="phone">
                        <i-input v-model="user.phone" :disabled="userFormDisabled" placeholder=""></i-input>
                    </form-item>
                    <form-item label="备注" prop="remark">
                        <i-input type="textarea" :autosize="{minRows: 2,maxRows: 5}" v-model="user.remark"
                                 :disabled="userFormDisabled"></i-input>
                    </form-item>
                    <form-item label="用户状态" prop="isEnabled">
                        <i-switch :true-value="Number(1)" :false-value="Number(0)" v-model="user.isEnabled"
                                  size="large">
    ...
    <script type="text/javascript">
        ...
        var appData = {
            ...
            userValidate: {
                username: [
                    ...
                    {type: 'string', min: 4, message: '登录名必须是4个以上的字符组成', trigger: 'blur'},
                    {validator: _u.validate_char, trigger: 'blur', message: '用户名取值由字母组成'},
                    {validator: func.usernameDump, trigger: 'blur', message: '用户名已重复'}
                ],
                displayName: [
                    {required: true, message: '输入显示名不能为空', trigger: 'blur'},
                    {type: 'string', min: 2, message: '显示名必须是2个以上的字符组成', trigger: 'blur'}
                ],
                password: [
        ...
        func.refreshList = function () {
            _$.ajax({
                url: '<@c.rootPath/>' + api.sysuser_query,
                dataType: 'json',
                type: 'POST',
            ...
            App.$refs['userForm'].validate(function (valid) {
                if (valid) {
                    _$.ajax({
                        ...
                        dataType: 'json',
                        type: 'POST',
                        data: {user: JSON.stringify(appData.user)},
                        success: function (_r, _s) {
                            if (_r.success) {
